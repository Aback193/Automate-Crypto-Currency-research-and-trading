...
classifier = "load"  # = retrain to retrain model if exists or create new one; = load to laod existent model;
...
indicatorsToUse = [
    ...
    "ADX",
    "CMF",
]  # ['RSI','STOCH','Williams','MACD','ROC','OBV','ADX'] # ['RSI','STOCH','Williams'] highest weight, add 'CMF' for crypto but not for Stocks
coin = "ETH"
    ...
    if sheet_name in workbook.sheetnames:
        ...
        for i in range(2, row_len + 1):
            if sheet["B" + str(i)].value in total_sentiment:
                total_sentiment[sheet["B" + str(i)].value] = total_sentiment[
                    sheet["B" + str(i)].value
                ] + float(sheet["C" + str(i)].value)
            else:
                total_sentiment[sheet["B" + str(i)].value] = float(
                    sheet["C" + str(i)].value
                )
        for i, (k, v) in enumerate(total_sentiment.items()):
            total_sentiment[k] = float(v) / float(
                (row_len - 1) / total_sentiment.__len__()
            )
...
def getBinanceHistoricalData(tickerPair, tf, startDate):
    try:
        ...
        hist_df["Open Time"] = pd.to_datetime(
            hist_df["Open Time"] / 1000, unit="s"
        ...
        )  # Convert String columns to Float64
        return hist_df
...
def getInitialHistoricalData():
    print(
        "\nFetching Binance history for {} on {} timeframe".format(
            tickerBinance, timeframe
        )
    )
    ...
    df_temp.to_csv(
        subfolderModels + "history_binance_" + tickerBinance + "_" + timeframe + ".csv"
    )
    return df_temp
...
def save_model(model):
    joblib.dump(
        model, subfolderModels + "random_forest_" + ticker + "_" + timeframe + ".joblib"
    )
...
def freshIndicatorCalculations():
    try:
        ...
        print(
            "\nFetching Binance history for {} on {} timeframe for last 2 days".format(
                tickerBinance, timeframe
            )
        )
        if not len(df_try) == 0:
...
def livePredicitonsBinance(freshIndicators):
    ...
    elif str(predict[0]) == "0.0" and (
        total_sentiment[ticker_key] > 0.4 and total_sentiment[ticker_key] < 0.7
    ):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if timeframe == "15m":
            ...
            while not (
                any(x == current_time_min for x in minutes) and current_time_sec == "01"
            ):
                now = dtn.now()
                current_time_min = now.strftime("%M")
                current_time_sec = now.strftime("%S")
