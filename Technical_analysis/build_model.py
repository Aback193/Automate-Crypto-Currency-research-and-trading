    ...
    df_new = pd.DataFrame(
        [
            [
                ticker,
                df.loc["Up"]["precision"],
                df.loc["Down"]["precision"],
                accuracyTotal,
            ]
        ],
        columns=["TICKER", "UP", "DOWN", "TOTAL"],
    )
...
def buildRandomForestModel(df, ticker, timeframe):
    ...
    df["Price_change_%_final"] = np.where(
        filterDown, -1.0, np.where(filterNeutral, 0.0, 1.0)
    )
    ...
    report = classification_report(
        y_true=y_test, y_pred=y_pred, target_names=target_names, output_dict=True
    )
    ...
    feature_imp = pd.Series(
        rand_frst_clf.feature_importances_, index=X.columns
